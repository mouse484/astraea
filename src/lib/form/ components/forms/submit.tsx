...
export default function Submit({ children, checkDirty = false, ...properties }: Props) {
  const form = useFormContext()
  return (
    <form.Subscribe selector={state => [state.canSubmit, state.isDirty, state.isSubmitting]}>
      {([canSubmit, isDirty, isSubmitting]) => (
        <Button type="submit" disabled={!canSubmit || (checkDirty && !isDirty) || isSubmitting} {...properties}>
